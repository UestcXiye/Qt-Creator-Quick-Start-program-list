...
{
    // QString fileName = QFileDialog::getOpenFileName(this, tr("文件对话框"), "C:", tr("图片文件(*png
    // *jpg);;文本文件(*txt)")); qDebug() << "fileName: " << fileName << endl;
...
void MyWidget::on_pushButton_6_clicked()
{
    ...
    //获取字符串
    QString string =
        QInputDialog::getText(this, tr("输入字符串对话框"), tr("请输入用户名："), QLineEdit::Normal, tr("admin"), &ok);
    if (ok)
        qDebug() << "string: " << string;
    //获取整数
    int value1 =
        QInputDialog::getInt(this, tr("输入整数对话框"), tr("请输入-1000到1000之间的数值"), 100, -1000, 1000, 10, &ok);
    if (ok)
        qDebug() << "value1: " << value1;
    //获取浮点数
    double value2 = QInputDialog::getDouble(this, tr("输入浮点数对话框"), tr("请输入-1000到1000之间的数值"), 0.00,
                                            -1000, 1000, 2, &ok);
    if (ok)
    ...
    items << tr("条目1") << tr("条目2");
    //获取条目
    QString item = QInputDialog::getItem(this, tr("输入条目对话框"), tr("请选择或输入一条条目"), items, 0, true, &ok);
    if (ok)
        qDebug() << "item: " << item;
}
