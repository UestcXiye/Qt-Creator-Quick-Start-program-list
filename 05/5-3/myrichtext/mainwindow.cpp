...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    format.setBorderBrush(Qt::red);                      //设置边界颜色
    format.setBorder(3);                                 //设置边界宽度
    ...
    ui->mainToolBar->addAction(action_textBlock);  //在工具栏添加动作

    QAction* action_font = new QAction(tr("字体"), this);
    action_font->setCheckable(true);
    connect(action_font, &QAction::toggled, this, &MainWindow::setTextFont);
    ui->mainToolBar->addAction(action_font);  //在工具栏添加动作
}
...
void MainWindow::showTextBlock()  //遍历所有文本块
{
    QTextDocument* document = ui->textEdit->document();
...
void MainWindow::setTextFont(bool checked)  //设置字体样式
{
    if (checked)  //如果处于选中状态
    {
